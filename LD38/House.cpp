...
void House::setTheta(const float& t)
{
	theta = t;
}
...
{
	float x = m_level->getPlanet()->getPosition().x + cosf(theta) * m_level->getPlanet()->getRadius();
	float y = m_level->getPlanet()->getPosition().y + sinf(theta) * m_level->getPlanet()->getRadius();
	setPosition({ x, y });
